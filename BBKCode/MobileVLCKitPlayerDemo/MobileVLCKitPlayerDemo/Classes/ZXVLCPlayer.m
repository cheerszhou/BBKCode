...
@interface ZXVLCPlayer ()
@property (nonatomic, strong) VLCMediaPlayer *player;
@property (nonatomic, nonnull, strong) ZXVideoControlView *controlView;
@property (nonatomic, assign) CGRect originFrame;
@end

@implementation ZXVLCPlayer
...
- (void)showInView:(UIView *)view {
    NSAssert(_mediaURL != nil, @"ZXVLCPlayer Exception: mediaURL could not be nil!");
...
- (void)controlViewFingerMoveDown {
    self.controlView.volumeSlider.value -= 0.05f;
}
...
- (ZXVideoControlView*)controlView {
    if (!_controlView) {
        _controlView = [[ZXVideoControlView alloc]initWithFrame:self.bounds];
...
- (void)setIsFullscreenModel:(BOOL)isFullscreenModel {
    ...
    if (isFullscreenModel) {
        ...
        CGFloat height = kZXSCREEN_BOUNDS.size.width;
        CGFloat width = kZXSCREEN_BOUNDS.size.height;
        ...
        [UIView animateWithDuration:kVideoPlayerAnimationTimeinterval animations:^{
            if (UIDeviceOrientationIsLandscape([UIDevice currentDevice].orientation)) {
                ...
                self.frame = self.frame = kZXSCREEN_BOUNDS;
            }
        }];
    }else{
        [UIView animateWithDuration:kVideoPlayerAnimationTimeinterval animations:^{
